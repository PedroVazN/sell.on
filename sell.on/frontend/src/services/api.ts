...
export interface Notification {
  ...
  title: string;
  message: string;
  type: 'goal_achieved' | 'goal_milestone' | 'goal_created' | 'goal_updated' | 'goal_completed' | 'system' | 'warning' | 'info' | 'notice' | 'chat_message' | 'client_access_request' | 'proposal_followup';
  priority: 'low' | 'medium' | 'high' | 'urgent';
  recipient: string;
  sender: string;
...
class ApiService {
  ...
  async getProposalTasks(params?: {
    status?: string;
    pending?: boolean;
    proposalId?: string;
  }): Promise<
    ApiResponse<ProposalTask[]> & {
      meta?: { followUpDays: number; pendingCount: number };
    }
  > {
    ...
    const qs = q.toString();
    return this.request<ProposalTask[]>(`/proposal-tasks${qs ? `?${qs}` : ''}`);
  }

  async completeProposalTask(
    taskId: string,
    notes?: string
  ): Promise<ApiResponse<ProposalTask>> {
    return this.request<ProposalTask>(`/proposal-tasks/${taskId}/complete`, {
      method: 'PATCH',
      body: JSON.stringify(notes ? { notes } : {}),
    });
  }

  async runProposalFollowUpScan(): Promise<ApiResponse<FollowUpScanResult>> {
