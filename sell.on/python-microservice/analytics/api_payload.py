"""Monta o dicionário da API /analyze (schema v2) — usado pelo Flask e pelo script stdin."""
...
def empty_analysis_payload(counters: dict | None) -> dict:
...
def build_analysis_payload(proposals: list, counters: dict | None = None) -> dict:
    """Mesmo contrato JSON do POST /analyze (sem Flask)."""
    counters = counters or {}
    df = proposals_to_dataframe(proposals)
    if df.empty:
        return empty_analysis_payload(counters)
